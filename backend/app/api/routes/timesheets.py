...
def to_out(r: TimesheetRecord) -> TimesheetOut:
    return TimesheetOut(
        ...
        other_leave_dates=r.other_leave_dates or [],
        ...
        other_leave_count=r.other_leave_count,
    ...
    period_records = (
        await db.execute(
            select(TimesheetRecord).where(
                ...
                TimesheetRecord.year == year,
                TimesheetRecord.matched_employee_pk.isnot(None),
            )
        )
    ).scalars().all()
...
@router.get("/by-period", response_model=list[TimesheetExportOut])
...
async def list_records(
    ...
    if month is not None and year is None:
        raise HTTPException(400, "year is required when month is set")
...
async def update_record(record_id: str, body: TimesheetUpdate, db: AsyncSession = Depends(get_db)):
    ...
    if body.other_leave_dates is not None:
        r.other_leave_dates = _clean(body.other_leave_dates)
    ...
    r.other_leave_dates = cleaned["other"]
